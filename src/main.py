...
class TextEditWindow(Gtk.Window):
    ...
    def create_textview(self):
        ...
        self.tag_found = self.textbuffer.create_tag("found", background="orange")

    def create_buttons(self):
        check_editable = Gtk.CheckButton(label="Povolit editaci")
        check_editable.set_active(True)
        check_editable.connect("toggled", self.on_editable_toggled)
        self.grid.attach(check_editable, 0, 2, 1, 1)

        check_cursor = Gtk.CheckButton(label="Zobrazit kurzor")
        ...
        radio_wrapnone = Gtk.RadioButton.new_with_label_from_widget(None, "Nic nezahrnovat")
        self.grid.attach(radio_wrapnone, 0, 3, 1, 1)

        radio_wrapchar = Gtk.RadioButton.new_with_label_from_widget(
            radio_wrapnone, "Zahrnout znaky"
        )
        self.grid.attach_next_to(
...
class DialogWindow(Gtk.Window):
    ...
    def on_open_clicked(self, button):
        print('Zmeny nebudou zahozeny.')

    def on_close_clicked(self, button):
        print("Zahazuji zmeny a zaviram textedit...")
        Gtk.main_quit()
